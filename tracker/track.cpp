...
int main(int argc, char** argv) {
  ...
  if(1){
    if (file_type == "image") {
      ...
      */
                my_width = d[5]* img.cols - d[3]* img.cols;
                my_height = d[6] * img.rows - d[4] * img.rows;
      
                x_avg =( d[3]* img.cols + d[5] *img.cols)/2;
                y_avg =( d[4]* img.rows + d[6] *img.rows)/2;
                int baseline = 0;
                string text;
                ...
                if (d[1] == 15){
                    ...
                    rectangle(img, cv::Point(d[3]*img.cols,d[4]*img.rows), 
